...
def cleanup_old_files(directory: Path, keep: int = 3) -> None:
    """
    Keep only the 'keep' newest combined files in the directory.

    Args:
        directory: Path to the directory containing files to clean.
        keep: Number of most recent files to preserve (default: 3).
    """
    if not directory.exists():
        return

    # Patterns for combined files generated by batch_fetch
    patterns = ["combined_*.json", "combined_*.csv"]
    for pattern in patterns:
        # Sort by modification time (newest first)
        files = sorted(list(directory.glob(pattern)), key=os.path.getmtime, reverse=True)
        if len(files) > keep:
            for old_file in files[keep:]:
                try:
                    old_file.unlink()
                    logging.info(f"Cleaned up old combined file: {old_file.name}")
                except Exception as e:
    ...
    try:
        ...
        for combined_dir in combined_dirs:
            if combined_dir.exists():
                # Clean up old combined files in this directory before processing (default to keeping 3)
                cleanup_old_files(combined_dir, keep=int(config.get('keep_combined_files', 3)))
