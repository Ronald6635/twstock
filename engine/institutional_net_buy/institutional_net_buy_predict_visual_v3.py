"""
使用訓練好的 V3 動態特徵模型進行特定個股預測與視覺化 (40天精準全對齊版)
==================================================================================
"""
    ...
    for col in columns:
        ...
        if col in string_cols:
            if arr.dtype.kind in ("S", "a"):
                ...
                data_dict[col] = np.array([x.decode("utf-8", errors="replace") if isinstance(x, bytes) else str(x) for x in arr])
...
def main():
    ...
    parser.add_argument("--scaler-path", type=str, required=True, help="V3 產生的 scaler_dimX.npz 路徑")
    ...
    if all_df.empty:
        print("TFRecord 中沒有資料。")
        return

    # 修正: 確保 date 欄位為 datetime 型別以供 .dt 使用
    all_df["date"] = pd.to_datetime(all_df["date"])
    
    # 過濾出最新日期有出現在 TFRecord 中的股票
    all_df["stock_id"] = all_df["stock_id"].map(normalize_stock_id)
    all_df = all_df[all_df["stock_id"].isin(target_ids)].sort_values(["stock_id", "date"]).reset_index(drop=True)
    ...
    for stock_id, stock_group in all_df.groupby("stock_id"):
        stock_id_str = normalize_stock_id(stock_id)
        stock_group = stock_group.reset_index(drop=True)
        
        # Skip stocks with insufficient data
        if len(stock_group) < args.window_size + FUTURE_DAYS: 
            continue
        ...
        # Process predictions (use .at[] for list assignments to avoid pandas interpretation issues)
        for idx_pos, pred_values in zip(valid_indices, preds_denorm):
            ...
            # Use .at[] for list assignment (treats list as single object)
        ...
        # 6. Visualization (with proper date formatting)
        plot_history_len = min(DEFAULT_PLOT_HISTORY_LEN, len(stock_group))
        if plot_history_len <= args.window_size:
            continue
        ...
        stock_name = str(stock_group["stock_name"].iloc[-1]) if "stock_name" in stock_group.columns else ""
        fig.suptitle(f"股票 {stock_id_str} ({stock_name.strip()}) 籌碼機器學習「時序全對齊」決策看板", fontsize=15, fontweight='bold')
        ...
        for i, conf in enumerate(hist_confidence):
            ax2.axvspan(x_indices[i] - 0.5, x_indices[i] + 0.5, 
                       alpha=0.2, color=get_confidence_color(conf), zorder=0)
        ...
        plt.tight_layout()
        safe_name = ''.join(c for c in stock_name.strip() if c not in r'\\/*?:"<>|')
        filename = f"{stock_id_str}_{safe_name}.png"
        
        try:
            plt.savefig(output_dir / filename, dpi=300)
            print(f"成功輸出 40 天完美對齊回測圖: {filename}")
