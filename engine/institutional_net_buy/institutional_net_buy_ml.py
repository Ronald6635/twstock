...
主要功能：
1. 從 TFRecord 讀取數據，進行特徵工程和標準化處理。
2. 建立一個適合時序數據的 Dilated CNN 模型架構。
3. 提供多種訓練損失選項，包括 MSE、Huber 和 Quantile Loss。
4. 支援訓練/驗證集的時間序列切分，確保模型評估的真實性。
5. 實現訓練過程中的模型快取機制，避免重複計算特徵。
...
注意事項：
- 請確保已安裝 TensorFlow 2.x、Keras 和 PyTorch 等必要的 Python 套件。
- TFRecord 檔案應包含必要的欄位，如 'date', 'stock_id', 'close', 'foreign_net_buy', 'trust_net_buy' 等。
- 訓練過程中會自動計算滾動特徵和目標報酬率，並根據指定的轉換方式進行處理。
- 模型訓練完成後會儲存為 `.keras` 格式，方便後續的預測和部署。
- 若使用快取功能，請確保 TFRecord 的內容和 target 轉換參數與快取檔案一致，否則將自動重新計算特徵。
- 目標轉換選項說明：
  - "clip": 將報酬率限制在指定的上下界之間，適合去除極端值對訓練的影響。
  - "tanh": 使用雙曲正切函數對報酬率進行非線性縮放，適合處理具有長尾分布的目標。
  - "none": 不對報酬率進行任何轉換，直接使用原始值進行訓練。
- 損失函數選項說明：
    - "mse": 均方誤差，適合對稱分布的回歸問題。
    - "huber": Huber 損失，對於異常值具有一定的魯棒性。
    - "quantile": 分位數損失，適合預測特定分位數的趨勢，例如 q=0.75 可用於預測上升趨勢的強度。

升級說明：
- 新增了對報酬率目標的轉換選項，提供更靈活的訓練目標處理方式。
- 引入了模型快取機制，避免在多次實驗中重複計算相同的特徵，提升開發效率。
- 模型架構和訓練流程進行了優化，提升了訓練的穩定性和預測的準確性。

機器學習訓練流程總覽
----------------------
1. 從 TFRecord 讀取原始數據，並計算 11 個特徵信號。
2. 根據指定的 target_transform 參數對報酬率目標進行轉換處理。
3. 建立訓練和驗證集，確保時間序列的連續性和無資料洩漏。
4. 定義並訓練 Dilated CNN 模型，使用指定的損失函數和訓練參數。
5. 訓練完成後儲存模型，並可選擇性地儲存訓練過程中的特徵快取以供未來使用。
"""
